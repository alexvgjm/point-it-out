<script lang="ts">
    ...
    const rectOptions: PropEntry[] = [{
        ...
        default: "'orange'",
        types: ['string'],
        description: "Stroke color. A SVG/CSS valid color string i.e <strong>'#aa23c8'</strong> or <strong>'rgba(211, 17, 32, 0.5)'</strong>."
    },{
        property: 'round',
        default: '0',
        types: ['number', 'string', '<strong>object</strong>'],
        description: "Round borders as number (pixel), a string with any valid SVG value (i.e '20%') or an object with rx and ry properties explained below.",
        props: [{
            property: 'rx',
            default: '0',
            types: ['number', 'string'],
        ...
        default: '0',
        types: ['number'],
        description: "How separated (in pixels) from the target content is the rect. Can be negative. By default (0 padding) the rect surrounds perfectly the target's bounding rect."
    }]
...
<p>In Point it out, we call "pointer" to created and absolutely positioned 
    elements via the create() function.</p>
...
<section class="doc-section">
    ...
    <p>The first and most basic pointer. In addition to the options described above, this pointer has the following specific options:</p>
...
<section class="doc-section">
    ...
    <p>The create function returns a reference to a Pointer instance, an object 
    with some useful methods to manipulate it. Also contains a reference
    to the target element and the created raw HTMLElement.</p>
...
// or some methods. More of this later.
